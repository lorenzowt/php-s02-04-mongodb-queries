...
db.restaurants.find({grades: {$elemMatch: {grade: "A", score: 11, date: "2014-08-11T00:00:00.000Z"}}}, {_id: 0, restaurant_id: 1, name: 1, grades: 1});
...
db.restaurants.find({$or: [{"address.street": {$exists: false}}, {"address.street": null}]}, {_id: 0, address: 1});

// 29. Seleccionar documents on el valor de `coordinate` és de tipus Double. Mostrar el name, restaurant_id i coordinades.
db.restaurants.find({"location.coordinates": {$elemMatch: {$type: "double" }}}, {_id: 0, restaurant_id: 1, "location.coordinates": 1});
...
db.restaurants.find({name: {$regex: "mon", $options: "i"}}, {_id:0, name: 1, borough: 1, "locations.coordinates": 1});
